...
const nats_subject_roomurl = process.env.NATS_SUBJECT_ROOMURL ? process.env.NATS_SUBJECT_ROOMURL : "room-goodrooms-tky-rent";
...
getPriceFloat = (page) => {
  return page.$('//tr[@class="price"]/td')
    .then( td => td.innerText() )
    .then( priceStr => {
      const priceWoUnit = priceStr.match(/[\d,]+/);
      return parseInt(priceWoUnit[0].replace(/,/g, '')) / 10000
    })
}

getSizeFloat = (page) => {
  return page.$('//h3[text()="広さ"]/parent::th/following-sibling::td')
    .then( td => td.innerText() )
    .then( roomSizeStr => {
...
getFloorLevel = (page) => {
  // 築年月:2019年11月/鉄筋コン/5階建て/南東/入
  return page.$('//tr[@class="note"]/td')
    .then( td => td.innerText() )
    .then( noteStr => {
      const matchResult = noteStr.match(/(\d+)階建/);
      const floorTopLevelStr = matchResult[1]
      return { floorLevel: 2, floorTopLevel: parseInt(floorTopLevelStr) }
    })
}

getLocation = (page) => {
  return page.$('//div[@class="address"]')
    .then( div => div.innerText() )
    .then( addressStr => addressStr.replace(/\s/g, '') )
}
...
getBuiltYear = async (page) => {
  ...
  return page.$('//tr[@class="note"]/td')
    .then( td => td.innerText() )
    .then( noteStr => {
      const matchResult = noteStr.match(/築年月:(\d+)年/);
