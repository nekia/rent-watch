...
scanRoom = async (page) => {
  ...
  try {
    ...
    let roomPathArray = await page.$$('//article')
      .then((elms) => {
        const promises = [];
        for (elm of elms) {
          promises.push(elm.$('a').then((a) => a.getAttribute("href") ))
        }
        return Promise.all(promises);
      });
    
    const roomLinks = roomPathArray.map(path => `https://www.bluestudio.jp${path}`)
