...
scanRoom = async (page) => {
  ...
  try {
    const roomPathArray = await page.$$('//table[contains(@class, "room")]/tbody/tr[@class="clickableRow"]')
      .then((rows) => {
        const promises = [];
        for (row of rows) {
          promises.push(row.getAttribute("onclick")
            .then( link => {
              const addressPath = link.match(/'(.+)'/);
              return addressPath[1];
            }));
        }
        return Promise.all(promises);
      });

    const roomLinks = roomPathArray.map(path => `https://tokyo-designers.com${path}`)
...
(async () => {
  ...
  console.log(`##### Start - Tokyo Designers`);

  for (checkUrl of checkUrls) {
  ...
  console.log(`##### Done - Tokyo Designers`);
