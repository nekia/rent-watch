...
scanRoom = async (page) => {
  ...
  try {
    const roomPathArray = await page.$$('//div[contains(@class, "post-list")]').then((divs) => {
      const promises = [];
      for (div of divs) {
        promises.push(div.$('a'));
      }
      ...
      for (anchor of anchors) {
        promises.push(anchor.getAttribute("href"));
      }
      return Promise.all(promises)
    });

    const roomLinks = roomPathArray.map(path => `https://r-store.jp${path}`)
